...
int32_t xlnx_kernel_start(IVASKernel *handle, int start, IVASFrame *input[MAX_NUM_OBJECT], IVASFrame *output[MAX_NUM_OBJECT])
{
    ...
    uint low_threashold = 40;
    uint max_threashold = 255;
    printf ("saket edge  uv_h %d and uv_w %d\n", uv_h, uv_w);
    printf ("saket low-threashold %d and max-threashold %d\n", low_threashold, max_threashold);
    ...
    ivas_register_write(handle, &(low_threashold), sizeof(uint32_t), 0x38);               /* In threashold */
    ivas_register_write(handle, &(max_threashold), sizeof(uint32_t), 0x40);                   /* In Y8 height */
